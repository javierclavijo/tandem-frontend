...
function Search() {
  ...
  return (
    <div css={isDesktop ? baseAppContainerWithoutTabsCss : baseAppContainerWithTabsCss}>
      ...
      <main
        id="search-main"
        css={css`${isDesktop ? searchMain : searchMainMobile};
              overflow: auto;
              height: 100%;
              display: flex;
            ...
            `}
      >
        {/* Infinite scroll component. Includes the search panel and results. Its properties are
         assigned conditionally based on the selected search type. */}
        <InfiniteScroll
          next={searchType === searchTypeOptions.USERS ? fetchNextUsersPage : fetchNextChannelsPage}
          hasMore={searchType === searchTypeOptions.USERS ? hasNextUsersPage ?? false
            : hasNextChannelsPage ?? false}
          loader={<p>Loading...</p>}
          dataLength={searchType === searchTypeOptions.USERS ? usersData?.pages.length ?? 0
