...
import {
  descriptionSection, infoSection, listSection, listSectionHeader, profileImg,
} from '../styles';
...
export function UserInfo() {
  ...
  React.useEffect(() => {
    ...
    if (data && !isCurrentUser) {
      const isFriendOfCurrentUser = data?.friend_chats.some((chat) => chat.users.some((chatUser) => chatUser.id === user?.id));
        ...
        <section css={infoSection}>
          {isEditable && data
            ? (
              <>
                <ImageInput
              image={data.image} defaultImage={defaultImg}
            ...
            : (
              <>
                <img
              src={data.image ?? defaultImg} alt="" css={css`${profileImg};
                          grid-area: input;
                        `}
            />
                <p>{data?.username}</p>
            ...
            >
              {isEditable
                ...
                {data.languages.map((language) => (language.level === 'NA' ? (
                        <LanguageBadge
                            language={language.language} level={language.level} bg={colors.DARK}
                            key={language.id}
                          />
                      ) : (
                          <div
        ...
        <section css={listSection}>
          ...
          {!data?.friend_chats.length
            ? (
              ...
              >
                <p>
                  {isEditable ? 'You have not added any friends yet.'
        ...
        >
          ...
          {!data?.memberships.length
            ? (
              ...
              >
                <p>
                  {isEditable ? 'You have not joined any channels yet.'
        ...
        ? (
          ...
          >
            ...
            >
              Delete
{' '}
              {languages.find((l) => l.key === selectedDeleteLanguage.language)?.value}
              {' '}
              from your
              languages?
            </p>
            <div css={css`
                ...
                `}
            >
              <Button visible onClick={handleDeleteLanguage}>
                Delete
              </Button>
              <Button visible onClick={() => setSelectedDeleteLanguage(null)}>
                Cancel
              </Button>
            </div>
          </ReactModal>
