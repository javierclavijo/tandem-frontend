...
import {
  useChangeUserRole, useChannel, useDeleteChannel, useJoinChannel, useLeaveChannel,
} from './hooks';
          ...
          {data?.memberships.map((membership) => (membership.user
            ? (
              ...
              {userIsAdmin() && membership.role === 'U' ? (
                        <Button
          visible
          onClick={async () => handlePromoteUser(membership.url)}
        >
                                                         Promote
          <FastArrowUpBox
                                    color={colors.PRIMARY} height="1.5rem"
                                    width="1.5rem"
                                  />
        </Button>
                      )
          ...
          <Button
                                    visible
                                    onClick={async () => handleDemoteUser(membership.url)}
                                  >
                                                       Demote
                                    <FastArrowDownBox
